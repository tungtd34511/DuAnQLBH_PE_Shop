...
{
    public partial class ColorIndex : Form
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly IColorService _colorService;
        public GetColorPagingRequest Request { get; set; } = new();
        public PagedResult<Data.Entities.Color> Result { get; set; } = new();
        public ColorIndex(IServiceProvider serviceProvider, IColorService colorService)
        {
            InitializeComponent();
        ...
        private async void ColorIndex_Load(object sender, EventArgs e)
        {
            Result = await _colorService.GetPaging(Request);
            await LoadForm();
        }
        ...
        {
            var frmDetail = _serviceProvider.GetRequiredService<UpdateColor>();
            frmDetail.Color = await _colorService.GetById(Id);
        ...
        private async Task BtnHide_Click(string Id, bool status)
        {
            if (status)
            {
                if (MessageBox.Show("Bạn có muốn hiển thị màu sắc?", "PE-SHOP", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    if (result)
                    {
                        MessageBox.Show("Hiển thị màu sắc thành công !");
                        Result = await _colorService.GetPaging(Request);
                        await LoadViewTable();
                        await LoadMenuPaging();
                    }
                    ...
                    {
                        MessageBox.Show("Hiển thị màu sắc thất bại !");
                    }
                }
            }
            else
            {
                if (MessageBox.Show("Bạn có muốn ẩn màu sắc?", "PE-SHOP", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    var color = await _colorService.GetById(Id);
        ...
        private async Task<GetColorPagingRequest> GetPagingRequest()
        {
        ...
        {
            var frmDetail = _serviceProvider.GetRequiredService<CreateColor>();
            frmDetail.FormClosed += async (o, s) =>
            {
                Result = await _colorService.GetPaging(Request);
                await LoadViewTable();
