...
{
    public partial class Updatepromotion : Form
    {
        ...
        public Updatepromotion(IPromotionService promotionService)
        {
            InitializeComponent();
        ...
        private async Task AddCategories()
        {
            foreach (var category in CategoryForCreates)
            {
                ...
                var checkcate = new CheckBox()
                {
                    ...
                    Text = category.Name,
                    Checked = categoryIds.Contains(id),
        ...
        private async Task AddProducts()
        {
            foreach (var item in Products)
            {
                ...
                var checkproduct = new CheckBox()
                {
                    ...
                    Checked =productIds.Contains(id),
                    Font = new System.Drawing.Font("Segoe UI", 10F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point),
                    AutoSize = true,
                };
        ...
        {
            numSalePercent.Value =(decimal) Promotion.DiscountPercent;
            TxtName.Text = Promotion.Name ;
            TxtTitle.Text=Promotion.Title  ;
            cboxSelectAll.Checked=Promotion.ApplyForAll ;
            dateStarted.Value=Promotion.FromDate ;
            dateEnded.Value=Promotion.ToDate  ;
            categoryIds = Promotion.ProductCategoryIds.Split(" ").ToList();
            productIds = Promotion.ProductIds.Split(" ").ToList();
        }
        #endregion
        ...
        private async void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            if(txt != "") { MessageBox.Show(txt); }
            {
                ...
                if (await _promotionService.Update(Promotion))
                {
